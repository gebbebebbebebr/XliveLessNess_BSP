...
#define WriteTextF(format, ...) fprintf_s(fileConfig, format, __VA_ARGS__)
	...
	WriteText("\n# The port to base all other ports at assuming their port numbers are divisable by 100 with the remainder being the offset from the base port.");
	WriteText("\n# Note that this value is the base port, so a base of 65535 and an offset of anything greater than +0 is not going to be a valid port number (overflow).");
	WriteText("\n# Also note that if the Title attempts to use ports 1001 and 1201 for example then both offsets would be +1 from the base which becomes a problem.");
	WriteText("\n# The purpose of this base port is to allow multiple instances of the Title to run where each instance would be spaced +1000 from this base port.");
	WriteText("\n# TODO: Perhaps when not specified (0) this concept should not apply in order to support Titles with incompatible port ranges to this logic.");
	WriteText("\n");

	WriteText("\n# xlive_net_disable:");
	WriteText("\n# Valid values:");
	WriteText("\n#   0 - All network functionality related to xlive functions is enabled.");
	...
	WriteTextF("\nxlive_base_port = %hu", xlive_base_port);
